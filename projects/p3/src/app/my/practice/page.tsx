...
import { Zap, BookOpen, Mic, Trophy, TrendingUp, ChevronRight, Clock } from "lucide-react";
...
export default async function PracticePage() {
  ...
  const totalDue = books.reduce((sum, b) => sum + b.items.length, 0);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm">플래시카드, 퀴즈, 스피킹으로 실력을 키워보세요</p>
      </div>
      ...
      {totalDue > 0 && (
        <div className="bg-gradient-to-r from-purple-500 to-blue-500 rounded-2xl p-5 mb-8 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm font-medium">📅 오늘 복습할 단어</p>
              <p className="text-3xl font-bold mt-1">{totalDue}개</p>
              <p className="text-purple-100 text-sm mt-1">지금 시작하면 더 잘 기억돼요!</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-5">
          ...
          <p className="text-sm text-gray-500 mb-4">카드를 넘기며 단어를 암기해요. SM-2 간격 반복 알고리즘 적용</p>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-2xl p-5">
          ...
          <p className="text-sm text-gray-500 mb-4">4지선다 퀴즈로 실력을 테스트해보세요</p>
          <div className="text-xs text-green-600 font-medium">
            {stats.byMode["QUIZ"]
              ? `정답률 ${Math.round((stats.byMode["QUIZ"].correct / stats.byMode["QUIZ"].total) * 100)}%`
              : "아직 기록 없음"}
          </div>
        </div>
        <div className="bg-gradient-to-br from-orange-50 to-red-50 border border-orange-100 rounded-2xl p-5">
          ...
          <p className="text-sm text-gray-500 mb-4">발음을 연습하고 음성인식으로 확인해요</p>
          <Link
      ...
      {/* 단어장 선택 */}
      <div className="mb-6">
        <h2 className="text-lg font-bold text-gray-900 mb-4">단어장 선택</h2>
        {books.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-2xl">
            <BookOpen className="h-10 w-10 mx-auto text-gray-300 mb-3" />
            <p className="text-gray-500 text-sm">단어장이 없습니다.</p>
            <Link href="/my/vocab" className="text-blue-600 text-sm mt-2 inline-block">
              단어장 만들기 →
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {books.map((book) => {
              ...
              return (
                <div
                  key={book.id}
                  className="bg-white border border-gray-200 rounded-xl p-4 hover:border-purple-300 hover:shadow-sm transition-all"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-gray-900">{book.title}</h3>
                      <p className="text-xs text-gray-400 mt-0.5">
                        총 {book._count.items}개 단어
                        {dueCount > 0 && (
        ...
        <div>
          ...
          <div className="space-y-2">
            {stats.recent.slice(0, 5).map((r) => {
              const accuracy = r.totalItems > 0 ? Math.round((r.correctItems / r.totalItems) * 100) : 0;
              return (
                <div
                  key={r.id}
                ...
                >
                  <span className="text-2xl">
                    {r.mode === "FLASHCARD" ? "📇" : r.mode === "QUIZ" ? "🎯" : "🎤"}
                  </span>
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-800">
                      {r.mode === "FLASHCARD" ? "플래시카드" : r.mode === "QUIZ" ? "퀴즈" : "스피킹"}
                    </p>
                    <p className="text-xs text-gray-400">
                      {r.totalItems}개 · 정답 {r.correctItems}개 · {r.durationSec}초
                    </p>
                  </div>
