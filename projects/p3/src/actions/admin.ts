...
export async function adminApproveInstructor(profileId: string) {
  ...
  await Promise.all([
    notifyInstructorApproved(profile.userId),
    logger.info("ADMIN", `강사 승인: ${profile.realName}`, {
...
export async function adminRejectInstructor(
  ...
  await Promise.all([
    notifyInstructorRejected(profile.userId, reason.trim()),
    logger.warn("ADMIN", `강사 반려: ${profile.realName}`, {
      userId: profile.userId,
...
export async function adminApproveCourse(courseId: string) {
  ...
  await Promise.all([
    notifyCourseApproved(updatedCourse.instructor.userId, updatedCourse.title, courseId),
    logger.info("ADMIN", `강의 승인: ${updatedCourse.title}`, {
      meta: { courseId },
    }),
  ]);
...
export async function adminRejectCourse(courseId: string, reason: string) {
  ...
  await Promise.all([
    notifyCourseRejected(course.instructor.userId, course.title, reason.trim(), courseId),
    logger.warn("ADMIN", `강의 반려: ${course.title}`, {
      meta: { courseId, reason },
    }),
  ]);
